...
export const PredictionItem = ({ category, reserve_asset = 'base', aa_address, event, reserve = 0, reserve_decimals = 0, yes_decimals = 0, no_decimals = 0, draw_decimals = 0, yes_price = 0, no_price = 0, draw_price = 0, allow_draw, end_of_trading_period, candles, reserve_symbol, yes_symbol, result, waiting_period_length, feed_name, expect_datafeed_value, oracle, yes_team_id, no_team_id, yes_team, no_team, supply_yes = 0, supply_no = 0, supply_draw = 0, league_emblem, league, actualChampionship, type }) => {
  ...
  const exists = !!aa_address;

  const isSportMarket = oracle === appConfig.SPORT_ORACLE;
  const marketHasCrests = isSportMarket ? yes_team_id !== undefined && no_team_id !== undefined : false;

  const SecondWrapper = exists ? Link : Fragment
  ...
  return <Wrapper {...wrapperProps}>
    <SecondWrapper {...secondWrapperProps}>
      <Card className={styles.itemWrap} style={{ color: "#fff", opacity: isExpiry ? 0.5 : 1 }}>
        {league_emblem && (actualChampionship === 'all' || type === 'all') && <Tooltip title={league}><img className={styles.league} src={league_emblem} alt={league} /></Tooltip>}
        <Row gutter={10} align="middle">
          <Col md={{ span: 16 }} xs={{ span: 24 }} sm={{ span: 24 }} ref={infoWrapRef}>
            {exists && <Space className={styles.notifyWrap}>
            ...
            {!marketHasCrests ? <div className={styles.eventDesc}>
              {event}
            </div> : <Row gutter={8} align={exists && allow_draw && draw_coef ? "bottom" : 'middle'}>
              <Col md={{ span: 8 }} xs={{ span: 8 }} style={{ textAlign: 'center' }}>
                <Img src={[`https://crests.football-data.org/${yes_team_id}.png`, `https://crests.football-data.org/${yes_team_id}.svg`]} width={'50%'} />
                <div className={styles.teamWrap}>
                  <small style={{ color: appConfig.YES_COLOR }}>{yes_team}</small>
                </div>
              ...
              <Col md={{ span: 8 }} xs={{ span: 8 }} style={{ textAlign: 'center' }}>
                ...
                <div>
                  <small>{moment.unix(end_of_trading_period).format('ll')}</small>
                </div>
              ...
              <Col md={{ span: 8 }} xs={{ span: 8 }} style={{ textAlign: 'center' }}>
                ...
                />
                <div className={styles.teamWrap}>
                  <small style={{ color: appConfig.NO_COLOR }}>{no_team}</small>
                </div>
              ...
              <Col md={{ span: 6 }} xs={{ span: 12 }}>
                <Tooltip title="yes price">
                  <div style={{ color: appConfig.YES_COLOR }}>
                    ...
                    <div className={styles.infoValueInDollar}>{priceOrCoef === 'price' ? `$${+Number(yes_price * currentReserveRate).toFixed(2)}` : ((yes_price && currentReserveRate && yes_coef) ? `$${Number(yes_price * currentReserveRate * yes_coef).toFixed(2)}` : '-')}</div>
                  </div>
                </Tooltip>
              </Col>
              <Col md={{ span: 6 }} xs={{ span: 12 }}>
                <Tooltip title="no price">
                  <div style={{ color: appConfig.NO_COLOR }}>
                    ...
                    <div className={styles.infoValueInDollar}>{priceOrCoef === 'price' ? `$${+Number(no_price * currentReserveRate).toFixed(2)}` : ((no_price && currentReserveRate && no_coef) ? `$${Number(no_price * currentReserveRate * no_coef).toFixed(2)}` : '-')}</div>
                  </div>
                </Tooltip>
              </Col>
              <Col md={{ span: 6 }} xs={{ span: 12 }}>
                <Tooltip title="draw price">
                  ...
                  {allow_draw ? <div style={{ color: appConfig.DRAW_COLOR }}>
                    ...
                    <div className={styles.infoValueInDollar}>{priceOrCoef === 'price' ? `$${+Number(draw_price * currentReserveRate).toFixed(2)}` : ((draw_price && currentReserveRate && draw_coef) ? `$${Number(draw_price * currentReserveRate * draw_coef).toFixed(2)}` : '-')}</div>
                  </div> : <span style={{ color: appConfig.DRAW_COLOR }}>-</span>}
                </Tooltip>
              </Col>
            </Row>}
          </Col>

          {exists ? (infoHeight && dataForChart.length > 0 && <Col md={{ span: 8 }} xs={{ span: 24 }} sm={{ span: 24 }} style={{ display: 'flex', alignItems: 'center' }}>
